 ...
 * Fixed: replaced undefined 'lavender-light' with 'lavender' in gradient.
 */
...
function PostureStatus({ posture, loading }) {
  const map = {
    correct: {
      icon: '✓', label: 'Correct', sub: 'Great form — keep it up!',
      bg: 'bg-green-50', border: 'border-green-200',
      iconBg: 'bg-green-100', iconTxt: 'text-status-ok', txt: 'text-status-ok',
    },
    wrong: {
      icon: '✗', label: 'Needs Fix', sub: 'Check the highlighted joints.',
      bg: 'bg-red-50', border: 'border-red-200',
      iconBg: 'bg-red-100', iconTxt: 'text-status-bad', txt: 'text-status-bad',
    },
    idle: {
      icon: '·', label: 'Waiting', sub: 'Start the camera to begin.',
      bg: 'bg-surface', border: 'border-edge',
      iconBg: 'bg-lavender-pale', iconTxt: 'text-lavender', txt: 'text-ink-muted',
    },
  }
  ...
  return (
    <div className={`flex items-center gap-4 rounded-xl border px-4 py-3.5 ${cfg.bg} ${cfg.border}`}>
      ...
      <div>
        <p className={`font-display text-[1.4rem] font-extrabold leading-none ${cfg.txt}`}>{cfg.label}</p>
...
function ConfidenceBar({ value }) {
  ...
  // Fixed: removed undefined 'lavender-light' — use 'lavender-dark' to 'lavender'
